...
const METRIC_DEFS = [
  ...
  { key: "faithfulness" as const, label: "Faithfulness", color: "bg-teal-100 text-teal-700" },
  { key: "answerRelevance" as const, label: "Answer Relevance", color: "bg-cyan-100 text-cyan-700" },
  { key: "contextPrecision" as const, label: "Context Precision", color: "bg-orange-100 text-orange-700" },
  { key: "contextRecall" as const, label: "Context Recall", color: "bg-indigo-100 text-indigo-700" },
...
export function BenchmarkDashboard() {
  ...
  const handleRunBenchmark = () => startBenchmark()
  ...
  const comparedRuns = useMemo(() => {
    ...
    const r1 = runs?.find((r) => r.id === compareRun1)
    const r2 = runs?.find((r) => r.id === compareRun2)
    if (!r1 || !r2) return null
    return { r1, r2 }
  }, [compareRun1, compareRun2, runs])
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                    ...
                    <div className="mt-0.5 text-[11px] text-emerald-600">{latestRun.totalTests > 0 ? `${((latestRun.passedTests / latestRun.totalTests) * 100).toFixed(1)}%` : "0%"} of {latestRun.totalTests} cases</div>
                  </div>
                  <div className="rounded-xl border border-amber-100 bg-amber-50 p-4">
                    ...
                    <div className="mt-0.5 text-[11px] text-amber-600">{latestRun.totalTests > 0 ? `${((latestRun.partialTests / latestRun.totalTests) * 100).toFixed(1)}%` : "0%"} of {latestRun.totalTests} cases</div>
                  </div>
                  <div className="rounded-xl border border-rose-100 bg-rose-50 p-4">
                    ...
                    <div className="mt-0.5 text-[11px] text-rose-600">{latestRun.totalTests > 0 ? `${((latestRun.failedTests / latestRun.totalTests) * 100).toFixed(1)}%` : "0%"} of {latestRun.totalTests} cases</div>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-900">{latestRun.totalTests > 0 ? `${((latestRun.passedTests / latestRun.totalTests) * 100).toFixed(1)}%` : "—"}</span>
                  </div>
            ...
            {runs && runs.length > 0 ? (
              <div className="space-y-2">
                {[...runs].sort((a, b) => new Date(b.startedAt).getTime() - new Date(a.startedAt).getTime()).map((run) => (
                  <div key={run.id} className="flex items-center justify-between rounded-lg border border-gray-100 px-4 py-3 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-medium text-emerald-700">Hit Rate: {(run.aggregateMetrics.hitRate * 100).toFixed(1)}%</Badge>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="mb-6 flex items-end gap-4">
              <div className="flex-1">
                <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-gray-400">Run A</label>
                <SelectField
                  value={compareRun1}
                  onChange={setCompareRun1}
                  placeholder="Select a run…"
                  options={runs?.map((r) => ({ value: r.id, label: `${r.id.slice(0, 8)} — ${r.strategy} (${r.status})` })) ?? []}
                />
              </div>
              <div className="flex items-center pb-2"><GitCompare className="h-4 w-4 text-gray-400" /></div>
              <div className="flex-1">
                <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-gray-400">Run B</label>
                <SelectField
                  value={compareRun2}
            ...
            {comparedRuns ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead><tr className="border-b border-gray-100">
                    <th className="pb-3 pr-4 text-[11px] font-medium uppercase tracking-wider text-gray-400">Metric</th>
                    <th className="pb-3 px-4 text-[11px] font-medium uppercase tracking-wider text-gray-400">Run A</th>
                    <th className="pb-3 px-4 text-[11px] font-medium uppercase tracking-wider text-gray-400">Run B</th>
                    <th className="pb-3 px-4 text-[11px] font-medium uppercase tracking-wider text-gray-400">Delta</th>
                  </tr></thead>
                  <tbody>{METRIC_DEFS.map((def) => {
                    const valA = (comparedRuns.r1.aggregateMetrics as unknown as Record<string, number>)[def.key]
                    const valB = (comparedRuns.r2.aggregateMetrics as unknown as Record<string, number>)[def.key]
                    const delta = valA !== undefined && valB !== undefined ? valB - valA : undefined
                    const isPositive = delta !== undefined ? (def.isLatency ? delta < 0 : delta > 0) : undefined
                    return (
                      <tr key={def.key} className="border-b border-gray-50 last:border-0">
                        <td className="py-3 pr-4 text-xs font-medium text-gray-700">{def.label}</td>
                        <td className="py-3 px-4 text-xs text-gray-600">{formatMetricValue(def, valA)}</td>
                        <td className="py-3 px-4 text-xs text-gray-600">{formatMetricValue(def, valB)}</td>
                        <td className="py-3 px-4">
                          {delta !== undefined ? <span className={cn("text-xs font-medium", isPositive ? "text-emerald-600" : "text-rose-600")}>{isPositive ? "+" : ""}{def.isLatency ? `${Math.round(delta)}ms` : def.isTokens ? formatNumber(delta) : def.isCurrency ? `$${delta.toFixed(4)}` : `${(delta * 100).toFixed(2)}%`}</span> : <span className="text-xs text-gray-400">—</span>}
                        </td>
                      </tr>
                    )
                  })}</tbody>
